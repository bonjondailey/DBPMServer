...
#pragma warning  disable 1591
    ...
    public class PrinterHelper : IDisposable
    {
        ...
        public int Width
        {
            get
            {
                if (_innerPrinter.DefaultPageSettings.Landscape)
                    return
                        (int)
                            ConvertFromPrinterUnitsX(_innerPrinter.DefaultPageSettings.PrintableArea.Height,
                                ScaleModeConstants.VbTwips);
                return
                    (int)
                        ConvertFromPrinterUnitsX(_innerPrinter.DefaultPageSettings.PrintableArea.Width,
                            ScaleModeConstants.VbTwips);
            }
            set
            {
        ...
        public void PaintPicture(Image picture, PointF point)
        {
            ...
            PointF newP = new PointF((float)ConvertToPrinterUnitsX(point.X), (float)ConvertToPrinterUnitsY(point.Y));
        ...
        public void PaintPicture(Image picture, PointF point1, PointF point2)
        {
            ...
            PointF newP = new PointF((float)ConvertToPrinterUnitsX(point1.X), (float)ConvertToPrinterUnitsY(point1.Y));
        ...
        {
            PointF newP = new PointF((float)ConvertToPrinterUnitsX(point.X), (float)ConvertToPrinterUnitsY(point.Y));
            SizeF newSize = new SizeF((float)ConvertToPrinterUnitsX(size.Width), (float)ConvertToPrinterUnitsY(size.Height));
            InternalPaintPicture(picture, newP, newSize);
        }
        ...
        public void PaintPicture(Image picture, PointF point, SizeF size, RectangleF clippingInfo)
        {
            ...
            newImageInformation.clippingInfo =
               new RectangleF((float)ConvertToPrinterUnitsX(clippingInfo.X),
                              (float)ConvertToPrinterUnitsY(clippingInfo.Y),
                              (float)ConvertToPrinterUnitsX(clippingInfo.Width),
                              (float)ConvertToPrinterUnitsY(clippingInfo.Height));
        ...
        private void DrawLine(PointF point1, PointF point2, Color color)
        {
            ...
            PointF newP1 = new PointF((float)ConvertToPrinterUnitsX(point1.X), (float)ConvertToPrinterUnitsY(point1.Y));
            PointF newP2 = new PointF((float)ConvertToPrinterUnitsX(point2.X), (float)ConvertToPrinterUnitsY(point2.Y));
        ...
        public void Print(bool noNewLine, params object[] str)
        {
            ...
            for (int j = 0; j < str.Length; j++)
            {
                ...
                else if (str[j] is TabInfo)
                {
                    ...
                    double tabPos = tInfo.Column * GetColumnSize();
                    if (CurrentX > tabPos)
                        CurrentY += InternalTextHeight;
                ...
                else
                {
                    ...
                    newDataInfo.X = (float)ConvertToPrinterUnitsX(CurrentX);
                    newDataInfo.Y = (float)ConvertToPrinterUnitsY(CurrentY);
 ...
 */
        ...
        private static double ConvertFromPixelsX(double num, ScaleModeConstants scaleMode)
        {
            double res;
            switch (scaleMode)
            {
